---
title: 'Estadística Aplicada. Lab1: Aprendiendo a usar R y R-Studio'
...
data("climate") #importar base de datos de climas global albergado en el paquete gcookbook 
...
Llegados a este punto, vamos hacer algo de estadística descriptiva. Calculen promedio, varianza y desviación estandar a cada variable. Usen para ello la función `aggregate` del paquete `stats`. Para entender como usar esta función, busquen el documento de ayuda de esa función, colocando escribiendo `aggregate`en la barra de búsqueda del panel *Help*.

```{r, eval = FALSE}

#La función para calcular promedio es mean, para varianza es var, para desviación estándar es sd

summary(climate)

mean(climate$Anomaly10y, na.rm = T)

climate$Source <- factor(climate$Source)

aggregate(formula = Anomaly10y~Source, data = climate, FUN = mean)
