...
subtitle: "Lab2: Prueba de hipótesis simples"
...
3.  Contruye un gráfico de densidad probabilística para la variable `DBO` distinguiendo entre momento. Luego obtén un diagrama de cajas agrupando datos según el *Momento*. ¿cuál representación gráfica te parece más informativa? Explica la respuesta. Confirma que obtienes estos gráficos:

```{r, echo =  FALSE}
ggplot(datos2, aes(x = DBO))+
  geom_density(aes(group = Momento, fill = Momento), alpha=.3)+
  theme_bw()+
  ylab("Densidad Kernel")+
  xlab(expression(paste("DBO ", "(mg ", O[2], "/l/d)")))+
  ggtitle("Representación probabilística de la DBO")

boxplot(DBO~Momento, data = datos2, ylab = expression(paste("DBO ", "(mg ", O[2], "/l/d)")))
```
